...
def generate_pdf_report(department, month, year):
    ...
    # Title
    story.append(Paragraph(f"JSPL Condition Monitoring Cell (CMC) Report", title_style))
...
def generate_excel_report(department, month, year):
    ...
    ws1["A1"] = f"JSPL CMC Predictive Maintenance Schedule - {department.name} - {calendar_month_name(month)} {year}"
