...
class OilTestLogForm(forms.ModelForm):
    class Meta:
        model = OilTestLog
        fields = ['equipment', 'date', 'test_type', 'viscosity', 'moisture', 'nas_class', 'test_no', 'status', 'notification_no', 'login_by', 'sent_date', 'remarks']
        widgets = {
            'date': forms.DateInput(attrs={'type': 'date', 'class': 'input-mono'}),
            'test_type': forms.Select(attrs={'class': 'input-mono'}),
